...
async function sendCleanEmail() {
  console.log('Connecting to smtp.hostinger.com via IPv4 (family: 4, Port 465 SSL)...');
  
  const transporter = nodemailer.createTransport({
    host: 'smtp.hostinger.com',
    port: 465,
    secure: true,
    family: 4, // FORCE IPv4 to avoid ENETUNREACH IPv6
    auth: {
...
This is your Lagos 10K Multi-Sector Blended Outreach test notification sent directly to your real email: bethelmindrecruit@gmail.com.
...
Bethelmind Analytics Lagos`,
    html: `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0f172a; color: #ffffff; padding: 24px; border-radius: 12px; border: 1px solid #3b82f6;">
      ...
      <div style="background: rgba(255,255,255,0.05); padding: 16px; border-radius: 8px; margin: 16px 0; border: 1px solid rgba(255,255,255,0.1);">
        <p style="margin: 4px 0; color: #34d399;"><strong>✓ Delivery Status:</strong> 100% Active & Verified</p>
  ...
  console.log('✅ EMAIL DELIVERED TO bethelmindrecruit@gmail.com! Message ID:', info.messageId, 'Accepted:', info.accepted);
}

sendCleanEmail().then(() => process.exit(0)).catch(e => { console.error('Final error:', e); process.exit(1); });
