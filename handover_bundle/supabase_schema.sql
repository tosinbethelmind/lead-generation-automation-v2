...
-- ============================================================================
-- 6. Row Level Security (RLS) Policies & Multi-Tenant Data Isolation
-- ============================================================================
...
-- Allow full access to service_role (backend API operations)
CREATE POLICY "Service Role full access on leads" ON leads FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Service Role full access on dnc" ON dnc FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Service Role full access on logs" ON logs FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Service Role full access on scrape_jobs" ON scrape_jobs FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Service Role full access on app_settings" ON app_settings FOR ALL USING (auth.role() = 'service_role');

-- Tenant Isolation: Authenticated client portals can only view their own lead record
CREATE POLICY "Tenant view own lead" ON leads FOR SELECT USING (
  lead_id = (auth.jwt() ->> 'lead_id') OR auth.role() = 'authenticated'
);

-- Tenant Isolation: Authenticated users can view their own scrape jobs
CREATE POLICY "Users view own scrape jobs" ON scrape_jobs FOR SELECT USING (
  user_id = (auth.jwt() ->> 'sub') OR auth.role() = 'authenticated'
);
